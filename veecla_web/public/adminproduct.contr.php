...
class AdminProductContr extends AdminProduct
{
    ...
    private $upload_max_size3 = 2 * 1024 * 1024; // setting the max upload file size to 2MB
     ...
     // for the video
     private $video_name; //the image name
     private $video_type; // the image type
     private $video_size; // the image size
     private $video_temp; //the temporary location where the uploaded image is stored
     private $upload_video_folder = "./uploads/"; // the uplodas folder
     private $upload_video_max_size = 100 * 1024 * 1024; // setting the max upload file size to 2MB
 

    //property to hold an array of allowed image types

    private $allowed_image_types = ["image/jpg", "image/jpeg", "image/png", "image/gif"];
    private $allowed_video_types = ['video/mp4', 'video/x-matroska', 'image/gif'];
    ...
    public function __construct($product_name, $product_description, $product_price, $product_unit, $product_category, $product_info, $files, $files2, $files3, $files4)
    {
        ...
        $this->video_name = $files4['product_video']['name'] ?? '';
        $this->video_size = $files4['product_video']['size'] ?? 0;
        $this->video_temp = $files4['product_video']['tmp_name'] ?? '';
        $this->video_type = $files4['product_video']['type'] ?? '';
        ...
        if ($this->emptyInput() == true) {
            ...
            header("location: ../admin/addProduct.php?emptyInput");
            exit();
        }
        ...
        $this->videoNameValidation();
        $this->sizeValidation();
        $this->sizeValidation2();
        $this->sizeValidation3();
        $this->videoSizeValidation();
        $this->checkFile();
        $this->checkFile2();
        $this->checkFile3();
        $this->checkFile4();
        ...
        $this->addProducts($this->product_name, $this->product_description, $this->product_price, $this->product_unit, $this->product_category, $this->product_info, $this->newName(), $this->newName2(), $this->newName3(), $this->newName4());
    ...
    {
        if (empty($this->product_name) || empty($this->product_description) || empty($this->product_price) || empty($this->product_unit) || empty($this->product_category) || empty($this->product_info) || empty($this->newName()) || empty($this->newName2()) || empty($this->newName3()) || empty($this->newName4())) {
            return true;
        } else {
            return false;
        }
    ...
    private function isVideo()
    {
        ...
        $mime = finfo_file($finfo, $this->video_temp);
        finfo_close($finfo);
        if (!in_array($mime, $this->allowed_video_types)) {
            return $this->error = "Only [.mp4, .mkv] files are allowed";
        }
        return true;
    }
    ...
    private function videoNameValidation()
    {
        return $this->video_name = filter_var($this->video_name, FILTER_SANITIZE_STRING);
    ...
    private function videoSizeValidation(){
        if ($this->video_size > $this->upload_video_max_size) {
            return $this->error = "File is too large";
        }
    }
    ...
    private function checkFile4(){
        if (file_exists($this->upload_video_folder . $this->newName4())) {
            return $this->error = "File already exists in the folder";
        }
    }
    ...
    private function newName4()
    {
        return "veecla" . md5($this->video_name);
    }
    ...
    private function moveFile4(){
        if (!move_uploaded_file($this->video_temp, $this->upload_video_folder . $this->newName4())) {
            return $this->error = "There was an error, please try again";
        }
    }
}
