...
class AdminProduct extends Dbh
{
    ...
    protected function addProducts($product_name, $product_description, $product_price, $product_unit, $product_category, $product_info, $product_image, $related_image, $applied_image, $product_video)
    {
        $sql = "INSERT INTO products (product_name, product_description, product_price, product_unit, product_category, product_info, product_image, related_image, applied_image, product_video) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if (!$stmt->execute([$product_name, $product_description, $product_price, $product_unit, $product_category, $product_info, $product_image, $related_image, $applied_image, $product_video])) {
            $stmt = null;
            header("location: addProducts.php?error=connectionfailed");
            exit();
        }
    ...
    {
        $sql = "SELECT id, product_name, product_description, product_price, product_unit, product_category, product_info, product_image, related_image, applied_image, product_video FROM products WHERE is_product = ?";
        $stmt = $this->connection()->prepare($sql);
    ...
    protected function displayProducts5($is_product){
        $sql = "SELECT * FROM products WHERE is_product = ? ORDER BY dateAdded DESC LIMIT 9";
        $stmt = $this->connection()->prepare($sql);
